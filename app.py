    ...
    def updateRecords(self,Artist,Venue):
      ...
      if artist != null and venue != null:
        ...
        if int(year) < int(datetime.today().year):
          try:
            adata = list(artist.past_shows)
            vdata = list(venue.past_shows)
            adata.append(arecordVenue)
            vdata.append(vrecordArtist)
            currentCount = int(artist.past_shows_count) + 1
            vcurrentCount = int(venue.past_shows_count) + 1
            artist.past_shows = adata
            artist.past_shows_count = currentCount
            venue.past_shows = vdata
            venue.past_shows_count = vcurrentCount
            db.session.commit()
            print('committed')
            print(db.query.all())
          except:
          ...
          try:
            adata = list(artist.upcoming_shows)
            adata.append(arecordVenue)
            currentCount = int(artist.upcoming_shows_count) + 1
            artist.upcoming_shows = adata
            artist.upcoming_shows_count = currentCount
            vdata = list(venue.upcoming_shows)
            vdata.append(vrecordArtist)
            vcurrentCount = int(venue.upcoming_shows_count) + 1
            venue.upcoming_shows = vdata
            venue.upcoming_shows_count = vcurrentCount
            # db.session.commit()
          except:
...
def venues():
  ...
  return render_template('pages/venues.html', areas=new_data_list);
...
def show_venue(venue_id):
  ...
  new_data = Venue.query.get(venue_id)
  if new_data.genres[0] == '{':
      new_data.genres = new_data.genres.split('{')[1].split('}')[0].split(',')


  print(new_data.genres)
...
def show_artist(artist_id):
  ...
  new_data = Artist.query.get(artist_id)
  if new_data.genres[0] == '{':
    new_data.genres = new_data.genres.split('{')[1].split('}')[0].split(',')
  else:
    new_data.genres=new_data.genres.split(',')
...
def create_artist_submission():
  ...
  image_link=image_link,looking_for_venues=looking_for_venues,seeking_description=seeking_description,upcoming_shows=upcoming_shows,
  past_shows=past_shows,past_shows_count=past_shows_count, website_link=website_link,upcoming_shows_count=upcoming_shows_count)
